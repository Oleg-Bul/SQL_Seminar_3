...
-- 5
SELECT AVG(daily_typing_pages) AS Average
FROM employee_tbl;

-- 5.1
SELECT ROUND(AVG(daily_typing_pages), 1) AS Average
FROM employee_tbl;
-- ROUND округляет как в питоне (первый аргумент - что мы округялем, второй - сколько знаков после запятой)
...
GROUP BY age
HAVING age IN(24, 25);
-- отдельно с помощью HAVING выбраны только 2 группы
